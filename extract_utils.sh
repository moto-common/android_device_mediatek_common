...
# $7: is MTK common component - optional, default to false
#
...
function setup_vendor() {
    export IS_MTK_COMPONENT="$7"
    if [ -z "$IS_MTK_COMPONENT" ]; then
        local DEVICE="$1"
        if [ -z "$DEVICE" ]; then
...
//
// This file is generated by device/$VENDOR/$([ -z "$IS_MTK_COMPONENT" ] && echo ${DEVICE} || echo ${COMPONENT})/setup-makefiles.sh
...
EOF
            ...
            cat << EOF >> "$ANDROIDMK"
ifneq (,\$(filter $COMPONENT,\$(TARGET_COMMON_MTK_COMPONENTS)))
...
PRODUCT_SOONG_NAMESPACES += \\
    vendor/$VENDOR/$([ -z "$IS_MTK_COMPONENT" ] && echo ${DEVICE} || echo ${COMPONENT})
